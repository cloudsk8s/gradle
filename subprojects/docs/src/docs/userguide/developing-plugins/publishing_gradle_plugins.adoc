...
then the plugin ID should be associated with the author, for example by using the authors GitHub ID in a reverse domain pattern, like `io.github.johndoe`.
...
<7> Set a description that will be displayed on the portal. Although this is optional, it provides useful information to people would might
    want to use your plugin. Please give careful thought to the value of this property.
